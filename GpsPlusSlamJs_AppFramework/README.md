...
- **Accuracy is sub-meter, not centimeter — and it improves with motion.** The estimate has no evidence until the user moves, so the first moments of a session are its weakest; the fusion is what keeps locally-placed content sitting on its spot as the user walks around it. A convergence _time_ is deliberately not quoted here — see the note under "Alignment improves with motion" below.
...
- **Alignment improves with motion:** walking gives the solver a baseline, because only some alignments make the shape of the GPS track agree with the shape of the odometry track. How stable it feels depends on the device, the environment and how clean the GPS track is — but in our own outdoor tests it consistently held up well enough for content that needs to sit on a specific spot.
  - **On convergence time, deliberately no single number.** A "roughly 15 seconds" rule of thumb was quoted here until 2026-08-20 and is retracted: the only measurement this project has (51 recordings, private repo GpsPlusSlamJs_Investigation/docs/2026-07-25-0430-alignment-convergence-speed-findings.md) puts the median at around two minutes to come within a metre of the estimate's own final answer, with ~7 m still remaining at about twenty seconds. Those two statements measure different quantities — "visible drift vs raw GPS" is not "distance to the configuration's final answer" — which is exactly why quoting either loosely is a bad habit.
...
- **Anchors make placed content shareable:** When an object should also be tied to a GPS coordinate for replay, persistence, or sharing with other users, `createGpsAnchor` bootstraps from median GPS samples, keeps the Three.js object positioned from its GPS target inside `arWorldGroup`, and defers corrections until the object is off-screen. That deferral is **not** size-limited — a large alignment jump does not force an on-screen correction, because the jump is already absorbed smoothly for the whole view by the lerped `arWorldGroup.matrix` rather than by snapping one object under the user's gaze.
