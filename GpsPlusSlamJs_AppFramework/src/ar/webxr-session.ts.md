...
Built by [`createSceneHierarchy()`](ar-scene-hierarchy.ts.md), which lives in
its own module because replay uses it too — this module only consumes it.
...
- `arpose` local space is **WebXR** (X=East, Y=Up, Z=South):
  ...
  - **Replay:** receives recorded `odomPosition`/`odomRotation` from store subscriber (positions must
    be converted from NUE to WebXR via the library's `nueToWebXR()`, and rotations via `nueQuaternionToWebXR()`,
...
| `getRenderer()`             | `() => THREE.WebGLRenderer \| null`                                          | Get the session's renderer. **Read-only by convention:** the framework leaves tone mapping and output colour space at their neutral defaults because it has no opinion about a consumer's grading, so a consumer whose content was authored under a different grade must set — and restore — its own. Same handle answers "what does a frame cost" via `renderer.info.render`.                                                                                                                                                                                                                                                                                                                                                                                                            |
...
Removed in surface-reduction step 3 (zero consumers): `getLiveCss3dManager()` and `getCameraFrameCaptureSize()` were deleted; `AR_CAMERA_FOV`/`AR_CAMERA_NEAR`/`AR_CAMERA_FAR` became module-private constants — and have since moved with `createSceneHierarchy()` to [`ar-scene-hierarchy.ts`](ar-scene-hierarchy.ts.md).

**`nueToWebXR()` and `nueQuaternionToWebXR()` are no longer exported from here.** They moved to [`nue-webxr-conversions.ts`](nue-webxr-conversions.ts.md), which re-exports the library's implementations and takes **tuples** rather than `readonly number[]`. The API table above still listed the deleted `nueQuaternionToWebXR()` row after that move — raised in review on PR #231, and it mattered because `./ar/*` advertises `gps-plus-slam-app-framework/ar/webxr-session` as a public subpath, so a consumer following this table would deep-import a symbol that is not there.
...
- **arpose in replay** receives NUE odom positions converted to WebXR via the library's `nueToWebXR()` so the
...
- **Camera frustum (70° / 0.01 m / 200 m) is owned by [`ar-scene-hierarchy.ts`](ar-scene-hierarchy.ts.md)** — the single source of truth for live AR and replay, since both build their camera via `createSceneHierarchy()`. See that sidecar for the F2 rationale and the depth-precision constraint; the frustum test in `ar-scene-hierarchy.test.ts` pins the values on the constructed camera
...
- **Live-session-only getters (surface-reduction step 2):** `getScene()`/`getArWorldGroup()`/`getCamera()`/`getRenderer()` return the LIVE AR session's scene graph, set internally by `initAR()` and cleared by `resetWebXRState()` — they are `null` during desktop replay. The historical replay injection exports (`setScene`/`setArWorldGroup`/`setCamera`/`setArPose`/`getArPose` — replay-mode Risk R1) were deleted: replay owns its scene in `replay-scene.ts` (`getReplayState()`), and scene-reading visualizers accept a replay source via `gpsEventVisualizer.setSceneSource(...)` instead.
...
**`createSceneHierarchy()`:** moved out of this module — its contract tests now
live in [ar-scene-hierarchy.test.ts](ar-scene-hierarchy.test.ts).

**Live scene getters:**

- `getScene()` / `getArWorldGroup()` / `getCamera()` / `getRenderer()` / `getCurrentArPose()` return `null` before initialization, and again after teardown — `resetWebXRState()` clears the whole scene graph, so **none of these objects is reused across sessions**. A consumer restoring settings on them is being tidy, not preventing a leak.
- The behavioural `applyAlignmentMatrix` tests (alignment written verbatim to the live `arWorldGroup`, full-chain WebXR→NUE mapping, `nueToWebXR` replay composition) live in `webxr-session.alignment.test.ts` — they seed the module state through the real `initAR()` path (mocked `WebGLRenderer` + `navigator.xr`)
