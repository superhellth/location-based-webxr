...
import { TourLoadError } from './tour-load-error.js';
 ...
 * central-directory parse, manifest parsing, the asset provider, the warm +
 * switch — against a real HTTP server that speaks real 206/Content-Range and
 * can be toggled to refuse ranges, 404, 416, corrupt bytes or drop the
 * connection. It is the deterministic proof that a plain shared link becomes
 * usable content, that a range-refusing host still works via fallback, and
 * that a broken link fails cleanly instead of crashing. `caches` is
 * browser-only, so an in-memory cache store is injected; real Cache API +
 * browser CORS need a manual browser check.
 */
...
/**
 * A self-contained fixture manifest. `openRemoteTour` owns no "Tour" type, so
 * this mirrors what a real caller's own parser would hand back: `.assets`
 * (the only field this module reads) plus `id`/`waypoints`, standing in for
 * whatever else a real manifest carries, to pin that `openRemoteTour` returns
 * the parsed object verbatim rather than reshaping it.
 */
const sampleTour = {
  id: 'tour-castle',
  waypoints: [{ id: 'wp-1' }, { id: 'wp-2' }, { id: 'wp-3' }],
  assets: [
...
/** Stand-in for a real app's manifest parser — `openRemoteTour` never
 *  validates the manifest itself, only that `parseTour` doesn't throw. */
function parseTour(text: string): typeof sampleTour {
  return JSON.parse(text) as typeof sampleTour;
}

/**
 * A valid manifest (asset-gate is declared + referenced) whose gate filename
 * points at a file the zip does NOT contain — the asset-coverage check must
 * catch this at load, not hand back a manifest with a dangling reference.
 */
async function craftMissingAssetZip(): Promise<Uint8Array> {
  const badTour = {
    ...sampleTour,
    assets: [
  ...
  const blob = await packFilesAsZip({ path: 'tour.json', json: badTour }, [
    // note: ghost.png intentionally absent
    { path: 'assets/asset-intro.mp3', file: new File([INTRO_BYTES], 'x') },
  ]);
...
beforeAll(async () => {
  server = await startFixtureServer(
    sampleTour,
    assetFiles,
    new Map([['bad-assets', await craftMissingAssetZip()]])
  );
});
...
describe('openRemoteTour — unusable links fail cleanly', () => {
  ...
  for (const [path, cause] of cases) {
    it(`rejects ${path} as TourLoadError(${cause}) without crashing`, async () => {
      ...
      const err = await openRemoteTour(
        `${server.origin}${path}`,
        parseTour,
        opts
      ).then(
...
describe('openRemoteTour — reload reuses a completed cache', () => {
  it('opens a second time with no network once the copy is cached', async () => {
    ...
    const first = await openRemoteTour(url, parseTour, base);
    await first.cacheWarming; // populates the shared store

    const before = server.requestCount();
    const second = await openRemoteTour(url, parseTour, base);
  ...
  it('evicts a poisoned cached copy and recovers from the network', async () => {
    ...
    const before = server.requestCount();
    const opened = await openRemoteTour(url, parseTour, base);
    const after = server.requestCount();
    ...
    const reload = await openRemoteTour(url, parseTour, base);
