...
| `clearRefPointsCacheForAllScenarios()` | `async () => ClearRefPointsCacheResult` | Maintenance action: removes the cached `refPoints/` directory from every scenario in OPFS so the next scenario load re-imports ref points from `*.zip` recordings. Errors per scenario are collected; missing caches are not errors. |
