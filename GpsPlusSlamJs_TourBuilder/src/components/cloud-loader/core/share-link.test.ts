...
/**
 * Why these tests matter: authors paste the share link their cloud service put
 * on the clipboard, not the provider's raw download URL (§2.5.6). Each rewrite
 * here is the difference between a working tour and an HTML preview page
 * reaching zip.js as "corrupt". Equally load-bearing: anything that is not a
 * recognized share page must pass through byte-identical, or the layer would
 * corrupt direct URLs and proxy URLs that already work.
 */
