...
// deps: getUserWorldPos, getObjects, getZones, dispatch, config, movementEpsilonM?
// returns: { tick(), reset() }
```
...
`zones` snapshot, and `dispatch`es `setWaypointZone` for each transition. That is
**all** it does — no asset-provider, no Three.js (contract D15/§2.5). Everything
is injected, so the same driver runs unchanged from the demo and the composed app.
...
pass-throughs, one near-miss, one far), and runs `step()` over every sample. All
assertions are geometry-derived, not hand-tuned:
...
`proximity-driver.test.ts` (unit) covers dispatch of transitions, the
no-change/no-dispatch case, the movement-epsilon gate, the null-pose guard, and
