...
import { QR_OPTIONS, generateQr } from "./generate-qr.js";

/**
 * Why these tests matter: the QR is the hand-off from authoring to a visitor's
 * phone, and it is verified in the worst possible place — outdoors, by someone
 * who cannot debug it. So the contract worth pinning is narrow but absolute:
 * the URL reaches the encoder **byte-identical**. A helpful-looking trim or
 * re-encode here would silently corrupt a presigned `?tour=` value.
 *
 * `qrcode` is mocked rather than exercised: its encoding is its own project's
 * problem, and rendering a real SVG would test the library, not this wrapper.
 * (Module mock, not `vi.spyOn` on the namespace — an ESM default export's
