...
- **Text goes in with `textContent`, never a template string.** Tag keys and values come from OSM and rule ids from a publicly editable sheet. The framework’s `escape-html.ts` exists because this app already renders sheet-derived text into HTML sinks; the panel avoids the sink rather than escaping into one.
