...
- **The surface is RELATIVE, and the datum cancels.** The provider returns orthometric height (~53 m at Cologne) while the ENU frame puts the user at `y = 0`. The sampled height **at `centreEnu`** is subtracted from every read — at the window's centre, which is where the user is, rather than at the frame origin they may have walked away from. Taken at the origin instead, a user who has walked 40 m uphill stands 40 m above the scene's zero plane with the camera still framed at `y ≈ 10`. **AR mode needs the opposite** — absolute height against an ellipsoidal GNSS altitude, which is what the package's geoid model is for.
  - **That is now BUILT, and it is one scalar rather than a second sampler** (AR milestone 1). `sampleGrid` takes an optional `absoluteDatum: { undulationMetres }` and sets `datum = −N` instead of the window-centre height. Since `heightAt` returns `surfaceHeight − datum`, that yields `DEM_orthometric + N` — an ellipsoidal height, which is what the framework's GPS-world Y is (`calcRelativeCoordsInMeters` is called with `originAltitude = 0` everywhere, so scene `y = 0` is the WGS84 ellipsoid).
  - **There is NO origin term.** Draft 3 of the AR plan wrote `− ellipsoidalAltitudeAtOrigin` and was wrong by ~98 m at Cologne.
  - **The point is that the absolute datum does not depend on the window.** The window follows the user, so a window-centre datum moves mid-session and takes the whole scene's Y baseline with it — which is exactly what AR cannot have. A test pins that two windows 500 m apart over _sloped_ terrain report the same absolute datum and different relative ones.
