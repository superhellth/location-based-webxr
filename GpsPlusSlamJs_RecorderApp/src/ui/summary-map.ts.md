...
| Reference Point Markers | `VIS_COLORS` red `#ff6b6b` / green `#00ff00` | Drawn by the recorder-owned [draw-ref-point-markers.ts](draw-ref-point-markers.ts) helper — the SAME renderer the live/replay minimaps use via [ref-point-map-markers.ts](ref-point-map-markers.md) (2026-07-05). Red `📍 current`, green `📌 prior` by `timestamp` vs. `startTime`. |
